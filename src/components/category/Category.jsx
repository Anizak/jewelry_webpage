...
const Category = ({ toAdd, isCallback, callback }) => {
  const { state, dispatch } = useContext(ProductContext);
  const [filtredCategories, setFilteredCategories] = useState([]);
  const [filtredCategoriesWithoutContext, setFilteredCategoriesWithoutContext] =
    useState([]);
  const [localGender, setLocalGender] = useState(2);
  ...
  useEffect(() => {
    setFilteredCategories(
      state.categories.filter((item) => item.gender === state.gender)
    );
  }, [state.gender, state.categories]);

  useEffect(() => {
    setFilteredCategoriesWithoutContext(
      state.categories.filter((item) => item.gender === localGender)
    );

    setSelectedCategory(
      state.categories.filter((item) => item.gender === localGender)[0]
    );
  }, [localGender]);
  ...
  return (
    <div className={style.main}>
      <div className={style.btnBox}>
        <button
          onClick={isCallback ? () => setLocalGender(2) : () => selectGender(2)}
          className={style.btn}
        >
          <svg
            width="18"
          ...
          >
            <path
              ...
              fill={
                isCallback && localGender === 2
                  ? "rgba(0, 8, 193, 1)"
                  : !isCallback && state.gender === 2
                  ? "rgba(0, 8, 193, 1)"
                  : "#939393"
              }
            />
        ...
        <button
          onClick={isCallback ? () => setLocalGender(1) : () => selectGender(1)}
          className={style.btn}
        >
          <svg
            width="19"
          ...
          >
            <path
              ...
              fill={
                isCallback && localGender === 1
                  ? "rgba(0, 8, 193, 1)"
                  : !isCallback && state.gender === 1
                  ? "rgba(0, 8, 193, 1)"
                  : "#939393"
              }
            />
      ...
      <div className={style.categoryBox}>
        {isCallback
          ? filtredCategoriesWithoutContext.map((category) => {
              return (
                <CategoryItem
                  key={category.id}
                  item={category}
